...
TEST(HW5Test, TEST5) {
    ...
    cappuccino.add_side_item(std::make_shared<Cinnamon>(1));
    EXPECT_DOUBLE_EQ(cappuccino.price(), 60);
}

TEST(HW5Test, TEST6) {
    EspressoBased* esp{new Cappuccino{}};
    reinterpret_cast<Cappuccino*>(esp)->add_side_item(std::make_shared<Cookie>(1));
...
TEST(HW5Test, TEST7) {
    Cappuccino cappuccino;
    cappuccino.add_side_item(std::make_shared<Chocolate>(2));
...
TEST(HW5Test, TEST8) {
    Cappuccino cappuccino;
    cappuccino.add_side_item(std::make_shared<Water>(1));
    Cappuccino equal;
    equal.add_side_item(std::make_shared<Sugar>(1));
    equal = cappuccino;
...
TEST(HW5Test, TEST9) {
    Cappuccino equal;
    equal.add_side_item(std::make_shared<Sugar>(2));
    equal = equal;
...
TEST(HW5Test, TEST10) {
    ...
    reinterpret_cast<Cappuccino*>(esp)->add_side_item(std::make_shared<Cookie>(1));
    reinterpret_cast<Cappuccino*>(esp)->add_side_item(std::make_shared<Sugar>(2));
    std::vector<std::shared_ptr<Ingredient> >& sides = reinterpret_cast<Cappuccino*>(esp)->get_side_items();
